...
def mock_simulation():
    ...
    sim.clock.rate = 1.0
    sim.get_system_state.return_value = {f"Ast-{i:04d}": Vec2(i, i) for i in range(1000)}
    
    # Setup bodies provider
    bodies = mock.MagicMock()
    bodies.get_groups.return_value = {"AsteroidBelt": [f"Ast-{i:04d}" for i in range(1000)]}
    bodies.get_body.return_value = {"radius": 1000.0, "color": (200, 200, 200)}
    bodies.list_bodies.return_value = [f"Ast-{i:04d}" for i in range(1000)] + ["Sun", "Earth"]
    sim.bodies = bodies
...
class TestAsteroidLOD:
    ...
    def test_trail_lod_threshold_low(self, mock_log_scale, mock_map, mock_simulation, mock_viewport):
    ...
    @mock.patch("src.render.map_to_screen")
    @mock.patch("src.render.log_scale_size")
    def test_trail_lod_threshold_high(self, mock_log_scale, mock_map, mock_simulation, mock_viewport):
...
class TestAsteroidBatching:
    @mock.patch("src.render.map_to_screen")
    @mock.patch("src.render.log_scale_size")
    def test_batch_blits_called(self, mock_log_scale, mock_map, mock_simulation, mock_viewport):
        renderer = Renderer(mock_simulation, mock_viewport)
        surface = mock.MagicMock(spec=pygame.Surface)
