...
def test_src_frames_changelog_sprint_4():
    ...
    assert has_changelog_entry("src/frames.py", entry), f"Missing entry in src/frames.py: {entry}"
...
def test_src_simulation_changelog_sprint_4():
    ...
    for entry in entries:
        assert has_changelog_entry("src/simulation.py", entry), f"Missing entry in src/simulation.py: {entry}"
...
def test_src_bodies_changelog_sprint_4():
    ...
    assert has_changelog_entry("src/bodies.py", entry), f"Missing entry in src/bodies.py: {entry}"
...
def test_no_changelogs_in_tests():
    ...
    for py_file in py_files:
        ...
        assert "# CHANGELOG:" not in content, f"Forbidden # CHANGELOG: found in {py_file}"