...
def test_main_loop_execution() -> None:
  ...
  with unittest.mock.patch("pygame.display.set_mode") as mock_set_mode, \
       unittest.mock.patch("pygame.display.set_caption") as mock_set_caption, \
       unittest.mock.patch("pygame.init"), \
       unittest.mock.patch("pygame.event.get", return_value=[pygame.event.Event(pygame.QUIT)]), \
       unittest.mock.patch("src.main.SceneManager"), \
       unittest.mock.patch("src.main.TitleScene"), \
       unittest.mock.patch("src.main.Renderer"), \
       unittest.mock.patch("src.main.Diagnostics") as mock_diag_cls, \
       unittest.mock.patch("src.main.Simulation"), \
       unittest.mock.patch("pygame.time.Clock") as mock_clock_cls, \
       unittest.mock.patch("pygame.display.flip"), \
       unittest.mock.patch("pygame.quit"), \
...
def test_graceful_shutdown_quit() -> None:
    ...
    mock_manager = unittest.mock.Mock()
    result = handle_global_input(event, SimulationClock(), unittest.mock.Mock(), manager=mock_manager)
