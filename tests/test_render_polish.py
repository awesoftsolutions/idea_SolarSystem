...
from src.bodies import BodyData, BodyProvider
...
@pytest.fixture
def mock_bodies() -> MagicMock:
    """Mock BodyProvider with Earth data."""
    provider = MagicMock(spec=BodyProvider)
    earth_data: BodyData = {
        "primary": "Sun",
        "a": 1.0,
        "e": 0.0167,
...
@pytest.fixture
def mock_simulation(mock_bodies: MagicMock) -> MagicMock:
    """Mock Simulation with a clock and body provider."""
    sim = MagicMock(spec=Simulation)
    sim.clock = MagicMock(spec=SimulationClock)
    sim.clock.t_sim = 0.0
    ...
    # Ensure get_system_state is called with t
    sim.get_system_state.side_effect = lambda t: sim.get_system_state.return_value
    return sim
...
def test_trail_alpha_curve(renderer: Renderer, surface: pygame.Surface) -> None:
    ...
    with patch("pygame.gfxdraw.line") as mock_line, \
         patch("src.render.map_to_screen", return_value=Vec2(400, 300)), \
         patch("src.render.Frame"), \
         patch("src.render.Simulation.get_system_state", return_value={"Earth": Vec2(0, 0), "Sun": Vec2(0, 0)}):
        ...
        for i, call in enumerate(mock_line.call_args_list):
            ...
            assert abs(alpha - expected_alpha) <= 1, f"Alpha mismatch at segment {i}: got {alpha}, expected {expected_alpha}"
...
def test_adaptive_sampling_logic(renderer: Renderer, surface: pygame.Surface) -> None:
    ...
    with patch("src.render.scale_orbit_geometry", side_effect=mock_scale_orbit), \
         patch("pygame.draw.lines") as mock_lines, \
         patch("pygame.gfxdraw.aapolygon") as mock_aa, \
         patch("src.render.world_to_screen", return_value=Vec2(400, 300)):

        # Case 1: Small orbit
        renderer.simulation.bodies.get_body.side_effect = None
        renderer.simulation.bodies.get_body.return_value = {
            "primary": "Sun",
            "color": (255, 255, 255),
            "test_a_v": 10,
            "test_b_v": 10,
...
def test_60_fps_performance_budget(renderer: Renderer, surface: pygame.Surface) -> None:
    ...
    renderer.simulation.bodies.list_bodies.return_value = list(bodies_data.keys())
    renderer.simulation.bodies.get_body.side_effect = lambda name: bodies_data[name]
    renderer.simulation.bodies.__contains__.side_effect = lambda name: name in bodies_data
    renderer.simulation.get_system_state.return_value = state

    with patch("src.render.map_to_screen", return_value=Vec2(400, 300)), \
         patch("src.render.map_to_world", return_value=Vec2(0, 0)), \
         patch("src.render.Frame"), \
         patch("src.render.world_to_screen", return_value=Vec2(400, 300)):
        # Warm up
    ...
    assert avg_frame_time_ms < 40.0, f"Performance budget exceeded: {avg_frame_time_ms:.2f}ms per frame"